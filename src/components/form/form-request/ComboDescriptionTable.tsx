...
type ComboItem = {
    id: number;
    groupId: number;   // items with same groupId = alternatives (OR choices) for 1 component
    itemcode: string;
    itemname: string;
    unitPrice: number | "";
    qty: number | "";
...
/** Group rows by groupId, preserving insertion order */
...
// ─── Serialization ────────────────────────────────────────────────────────────
// Internal format: groups joined by " + ", alternatives within a group joined by " OR "
// Each item segment: "code|name|qty|price|unitPrice"
...
const parseItem = (part: string, groupId: number): ComboItem => {
    ...
    if (segments.length >= 5) {
        // Internal: code|name|qty|price|unitPrice
    ...
    if (segments.length === 4) {
        // Old internal: code|name|qty|price (no unitPrice)
    ...
    if (segments.length === 3) {
        // DB format: code|name|unitPrice (qty)
...
/** Dùng khi lưu DB: chuyển về format "code|name|unitPrice (qty) + ..." */
...
export default function ComboDescriptionTable({ defaultValue, onChange }: Props) {
    ...
    // totalQty counts only the first alternative of each group (alternatives are OR — not additive)
    const totalQty = groups.reduce((sum, g) => sum + (Number(g[0].qty) || 0), 0);
    // totalPrice uses max price per group for conservative validation
    ...
    const handleChange = (
        id: number,
        field: "itemcode" | "itemname" | "unitPrice" | "qty",
        value: string | number
    ) => {
        updateRows(
            rows.map((r) => {
    ...
    // Add a new component (new group)
    const handleAdd = () => {
        if (totalQty >= MAX_QTY) return;
        updateRows([...rows, createRow(nextGroupId(rows))]);
    };

    // Add an alternative (OR choice) to an existing group
    ...
    // Build flat render list with group metadata
                ...
                <Table>
                    <TableHeader>
                        <TableRow>
                            ...
                            <TableCell isHeader className="px-3 py-2 text-xs font-medium text-gray-500 dark:text-gray-400 w-[18%]">
                                Item Code
                            </TableCell>
                            <TableCell isHeader className="px-3 py-2 text-xs font-medium text-gray-500 dark:text-gray-400 w-[28%]">
                                Item Name
                            </TableCell>
                            <TableCell isHeader className="px-3 py-2 text-xs font-medium text-gray-500 dark:text-gray-400 w-[16%] text-right">
                                Unit Price
                            </TableCell>
                            <TableCell isHeader className="px-3 py-2 text-xs font-medium text-gray-500 dark:text-gray-400 w-[10%] text-center">
                                Qty
                    ...
                    <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                        {renderList.map(({ row, groupIdx, isFirst }) => {
                            const firstItemOfGroup = groups[groupIdx][0];
                            ...
                            return (
                                <TableRow
                                    key={row.id}
                                    className={`hover:bg-gray-50 dark:hover:bg-white/[0.02] ${!isFirst ? "bg-brand-50/30 dark:bg-brand-500/5" : ""}`}
                                >
                                    {/* No. / OR badge */}
                                    ...
                                    <TableCell className="px-3 py-2">
                                        <div className="flex items-center gap-1">
                                            ...
                                            <button
                                                ...
                                                onClick={() => setPickingRowId(row.id)}
                                                className="flex-shrink-0 p-1.5 rounded border border-gray-200 dark:border-white/[0.1] text-gray-400 hover:text-brand-500 hover:border-brand-500 transition-colors"
                                            >
                                                <ListIcon />
                                            </button>
                                        </div>
                                    ...
                                    <TableCell className="px-3 py-2">
                                        <Input
                                            ...
                                            onChange={(e) =>
                                                handleChange(row.id, "unitPrice", e.target.value === "" ? "" : Number(e.target.value))
                                            }
                                        />
                                    </TableCell>
                                    ...
                                    {/* Add OR button — only for first item of group */}
                                    <TableCell className="px-1 py-2 text-center">
                                        {isFirst && (
                                            <button
                                                type="button"
                                                title="Add alternative (OR)"
                                                onClick={() => handleAddAlternative(firstItemOfGroup.groupId)}
